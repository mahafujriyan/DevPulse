...
const apiTsEntry = path.join(root, "api/index.ts");
const rootEntry = path.join(root, "index.js");

// Remove accidental api/index.ts — Vercel compiles it and fails on bundled JS.
if (fs.existsSync(apiTsEntry)) {
  fs.unlinkSync(apiTsEntry);
  console.log("Removed api/index.ts (must not compile ncc bundle as TypeScript)");
}
